...
//VM bindings
  ...
  void vm_write(FinchVM* vm, const char* text)
  {
    printf("%s", text);
  }

  void reportError(FinchVM* vm, FinchErrorType type, 
  ...
  void readModuleComplete(FinchVM* vm, const char* module, FinchLoadModuleResult result)
  {
    if (result.source) {
  ...
  FinchLoadModuleResult readModule(FinchVM* vm, const char* module) 
  {
    ...
    // Add a ".finch" file extension.
    pathAppendString(filePath, ".finch");
  ...
  const char* resolveModule(FinchVM* vm, const char* importer, const char* module)
  {
...
//main helpers
  ...
  FinchInterpretResult runFile(FinchVM* vm, const char* path)
  {
    ...
    FinchInterpretResult result = finchInterpret(vm, module->chars, source);
  ...
  int handle_args(int argc, const char* argv[]) 
  {
    ...
    if (argc < 2)
    {
      printf("This is a Finch test runner.\nUsage: finch_test [file]\n");
      return WREN_EX_USAGE;
    }

    if (argc == 2 && strcmp(argv[1], "--version") == 0)
    {
      printf("finch_test is running on Finch version %s\n", WREN_VERSION_STRING);
      return 1;
    }
