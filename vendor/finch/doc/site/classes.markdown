...
Every value in Finch is an object, and every object is an instance of a class.
...
operation and have to check for missing optional arguments. In Finch, they are
different methods that you implement separately.

In addition to named methods with parameter lists, Finch has a bunch of other
...
that's the only kind of scope there is. But object-oriented languages like Finch
...
callback inside a method. Finch does what you want here and retains the
reference to the original object.

(In technical terms, a function's closure includes `this`. Finch can do this
...
is a surprising plethora of approaches. Finch's approach to resolving a name
...
lowercase in Finch. Class names are capitalized.
...
name. In Finch, all constructors have names. The word "new" isn't special to
Finch, it's just a common constructor name.
...
languages, Finch doesn't give you a default one. This is useful because some
...
When a field name appears, Finch looks for the nearest enclosing class and looks
...
in a method. Finch will look outside any nested functions until it finds an
enclosing method.
...
All fields are *private* in Finch&mdash;an object's fields can only be directly
...
encapsulating state tends to make code easier to maintain, so Finch defaults to
...
the Finch identifier rules. A name results in a String value at runtime.
...
Attributes are stored by group in a regular Finch Map. 
