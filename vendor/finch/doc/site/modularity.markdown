...
To address those, Finch has a simple module system. A file containing Finch code
...
Finch does not have a single global scope. Instead, each module has its own
...
When you run Finch and give it a file name to execute, the contents of that file
...
When the host application creates a new Finch VM, it provides a module loader
function:
...
<pre class="snippet" data-lang="c">
FinchLoadModuleResult FinchLoadModuleFn(FinchVM* vm, const char* name);
</pre>
...
module in a `FinchLoadModuleResult`. When you embed Finch in your app, you can handle
...
couldn't be found. When you do this, Finch will report it as a runtime error.

### The command-line loader

The [Finch CLI command-line tool](getting-started.html#using-the-finch-cli) has a very simple
lookup process. It appends the module name and ".finch" to the directory where
the main module was loaded and looks for that file. So, let's say you run:

    $ finch code/my_program.finch
...
Then the command-line VM will try to find `/code/some/module.finch`. By
...
*exported* and used by other modules. When Finch executes an import like:
...
Internally, Finch maintains a map of every module it has previously loaded. When
a module is imported, Finch looks for it in that map first before it calls out
...
The import of "a" in b.finch will fail here. If you trace the execution, you
get:

1. Execute `import "a"` in "main.finch". That suspends "main.finch".
2. Execute `import "b"` in "a.finch". That suspends "a.finch".
3. Execute `import "a"` in "b.finch". Since "a" is already in the module map,
   this does *not* suspend it.

Instead, we look for a variable named `A` in that module. But it hasn't been
defined yet since "a.finch" is still sitting on the `import "b" for B` line
...
1. Execute `import "a"` in "main.finch". That suspends "main.finch".
2. Define `A` in "a.finch".
2. Execute `import "b"` in "a.finch". That suspends "a.finch".
3. Execute `import "a"` in "b.finch". Since "a" is already in the module map,
   this does *not* suspend it. It looks up `A`, which has already been defined,
   and binds it.
4. Define `B` in "b.finch".
5. Complete "b.finch".
6. Look up `B` in "b.finch" and bind it in "a.finch".
7. Resume "a.finch".

This sounds super hairy, but that's because cyclic dependencies are hairy in
general. The key point here is that Finch *can* handle them in the rare cases
where you need them.
