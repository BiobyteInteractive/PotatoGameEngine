...
Finch's syntax is designed to be familiar to people coming from C-like languages
while being a bit simpler and more streamlined.

Scripts are stored in plain text files with a `.finch` file extension. Finch does
...
Unlike C, block comments can nest in Finch:
...
reserves. Here's what Finch has:
...
Identifiers that start with underscore (`_`) are special in Finch. They are used
to indicate [fields](classes.html#fields) in classes.

## Newlines

Newlines (`\n`) are meaningful in Finch. They are used to separate statements:
...
newline in the middle would trip it up. To handle that, Finch has a very simple
...
Finch uses curly braces to define *blocks*. You can use a block anywhere a
...
newlines are already significant in Finch, so it's not totally unreasonable. The nice
...
We'll talk about Finch's different expression forms and what they mean in the
...
same kind of expression is ordered. Finch mostly follows C, except that it fixes
