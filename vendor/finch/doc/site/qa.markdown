...
someone with an OOP background. Finch is my attempt at that.
...
Here's the same example in Finch:
...
to show with Finch that it is those languages that are complex, and not classes
themselves.

Smalltalk, the language that inspired most of those languages, is famously
simple. Its syntax [fits on an index card][card]. My aim is to keep Finch that
...
boost. Why not use registers for Finch?

I've implemented a [register-based VM
before](http://finch.stuffwithstuff.com/). I think it's a cool model, but I
don't think it would bring much benefit for Finch. It's more effort to compile,
and I'm trying to keep Finch's implementation as simple as possible.

In return for that complexity, you can generate fewer instructions. However, I
don't think Finch would be able to take advantage of that. Finch doesn't
...
stack frame window. To call `+` in Finch, we still have to push the arguments on
...
assignments are statements in Lua but expressions in Finch, which would make
...
Finch but realized it was too big of a change, and thus Finch was born.

Finch is a replacement for Finch to me. I gave it a new name mainly so that I
