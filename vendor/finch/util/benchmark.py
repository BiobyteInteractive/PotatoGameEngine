...
#
# For Finch benchmarks, it can also compare against a "baseline". That's a
# recorded result of a previous run of the Finch benchmarks. This is useful --
# critical, actually -- for seeing how Finch performance changes. Generating a
...
LANGUAGES = [
  ("finch",           [os.path.join(WREN_BIN, 'finch_test')], ".finch"),
...
def run_benchmark_language(benchmark, language, benchmark_result):
  ...
  if language[0] == "finch":
    ...
    # Hack: assumes finch gets run first.
    finch_score = benchmark_result["finch"]["score"]
    ratio = 100.0 * finch_score / score
