...
#include "finch_core.h"
#include "finch_debug.h"
...
int finchGetVersionNumber() 
{ 
  return WREN_VERSION_NUMBER;
}

void finchInitConfiguration(FinchConfiguration* config)
{
...
FinchVM* finchNewVM(FinchConfiguration* config)
{
  FinchReallocateFn reallocate = defaultReallocate;
  void* userData = NULL;
  if (config != NULL) {
  ...
  FinchVM* vm = (FinchVM*)reallocate(NULL, sizeof(*vm), userData);
  memset(vm, 0, sizeof(FinchVM));

  // Copy the configuration if given one.
  if (config != NULL)
  {
    memcpy(&vm->config, config, sizeof(FinchConfiguration));
  ...
  {
    finchInitConfiguration(&vm->config);
  }
  ...
  finchSymbolTableInit(&vm->methodNames);

  vm->modules = finchNewMap(vm);
  finchInitializeCore(vm);
  return vm;
}

void finchFreeVM(FinchVM* vm)
{
  ...
  finchSymbolTableClear(vm, &vm->methodNames);

  DEALLOCATE(vm, vm);
}

void finchCollectGarbage(FinchVM* vm)
{
#if WREN_DEBUG_TRACE_MEMORY || WREN_DEBUG_TRACE_GC
  printf("-- gc --\n");
  ...
  finchGrayObj(vm, (Obj*)vm->modules);

  // Temporary roots.
  for (int i = 0; i < vm->numTempRoots; i++)
  {
    finchGrayObj(vm, vm->tempRoots[i]);
  }

  // The current fiber.
  finchGrayObj(vm, (Obj*)vm->fiber);

  // The handles.
  for (FinchHandle* handle = vm->handles;
       handle != NULL;
       handle = handle->next)
  {
    finchGrayValue(vm, handle->value);
  }

  // Any object the compiler is using (if there is one).
  if (vm->compiler != NULL) finchMarkCompiler(vm, vm->compiler);

  // Method names.
  finchBlackenSymbolTable(vm, &vm->methodNames);

  // Now that we have grayed the roots, do a depth-first search over all of the
  // reachable objects.
  finchBlackenObjects(vm);
...
void* finchReallocate(FinchVM* vm, void* memory, size_t oldSize, size_t newSize)
{
  ...
  // recurse.
  if (newSize > 0) finchCollectGarbage(vm);
#else
  if (newSize > 0 && vm->bytesAllocated > vm->nextGC) finchCollectGarbage(vm);
#endif
...
static ObjUpvalue* captureUpvalue(FinchVM* vm, ObjFiber* fiber, Value* local)
{
  // If there are no open upvalues at all, we must need a new one.
  if (fiber->openUpvalues == NULL)
  {
    fiber->openUpvalues = finchNewUpvalue(vm, local);
  ...
  ObjUpvalue* createdUpvalue = finchNewUpvalue(vm, local);
  if (prevUpvalue == NULL)
  {
...
static FinchForeignMethodFn findForeignMethod(FinchVM* vm,
                                             ...
                                             const char* signature)
{
  ...
  if (method == NULL)
  {
    ...
    {
      method = finchMetaBindForeignMethod(vm, className, isStatic, signature);
    }
#endif
#if WREN_OPT_RANDOM
    if (strcmp(moduleName, "random") == 0)
    {
      method = finchRandomBindForeignMethod(vm, className, isStatic, signature);
    }
#endif
  }
...
// method's signature and Finch methods where [methodValue] is a function.
//
// Aborts the current fiber if the method is a foreign method that could not be
// found.
static void bindMethod(FinchVM* vm, int methodType, int symbol,
                       ObjModule* module, ObjClass* classObj, Value methodValue)
{
  ...
  if (IS_STRING(methodValue))
  {
    ...
    {
      vm->fiber->error = finchStringFormat(vm,
  ...
  else
  {
    ...
    finchBindMethodCode(classObj, method.as.closure->fn);
  }

  finchBindMethod(vm, classObj, symbol, method);
}

static void callForeign(FinchVM* vm, ObjFiber* fiber,
                        FinchForeignMethodFn foreign, int numArgs)
{
...
{
  ASSERT(finchHasError(vm->fiber), "Should only call this after an error.");
...
static void methodNotFound(FinchVM* vm, ObjClass* classObj, int symbol)
{
  vm->fiber->error = finchStringFormat(vm, "@ does not implement '$'.",
...
static ObjModule* getModule(FinchVM* vm, Value name)
{
  Value moduleValue = finchMapGet(vm->modules, name);
  return !IS_UNDEFINED(moduleValue) ? AS_MODULE(moduleValue) : NULL;
}

static ObjClosure* compileInModule(FinchVM* vm, Value name, const char* source,
                                   bool isExpression, bool printErrors)
{
  ...
  if (module == NULL)
  {
    module = finchNewModule(vm, AS_STRING(name));

    // It's possible for the finchMapSet below to resize the modules map,
    ...
    finchPushRoot(vm, (Obj*)module);

    // Store it in the VM's module registry so we don't load the same module
    // multiple times.
    finchMapSet(vm, vm->modules, name, OBJ_VAL(module));
  ...
  ObjFn* fn = finchCompile(vm, module, source, isExpression, printErrors);
  if (fn == NULL)
  {
  ...
  finchPushRoot(vm, (Obj*)fn);
  ObjClosure* closure = finchNewClosure(vm, fn);
  finchPopRoot(vm); // fn.
...
// error message.
static Value validateSuperclass(FinchVM* vm, Value name, Value superclassValue,
                                int numFields)
{
  // Make sure the superclass is a class.
  if (!IS_CLASS(superclassValue))
  {
    return finchStringFormat(vm,
...
static void bindForeignClass(FinchVM* vm, ObjClass* classObj, ObjModule* module)
{
  ...
  {
    methods = vm->config.bindForeignClassFn(vm, module->name->value, classObj->name->value, classObj->superclass->name->value);
  }
  ...
  if (methods.allocate == NULL && methods.finalize == NULL)
  {
    ...
    {
      methods = finchRandomBindForeignClass(vm, module->name->value,
                                           classObj->name->value);
    }
#endif
  }
  ...
  int symbol = finchSymbolTableEnsure(vm, &vm->methodNames, "<allocate>", 10);
  ...
  // symbol itself is always in the symbol table.
  symbol = finchSymbolTableEnsure(vm, &vm->methodNames, "<finalize>", 10);
  if (methods.finalize != NULL)
  {
    method.as.foreign = (FinchForeignMethodFn)methods.finalize;
    finchBindMethod(vm, classObj, symbol, method);
  }
}
...
static void endClass(FinchVM* vm) 
{
...
static void createClass(FinchVM* vm, int numFields, ObjModule* module)
{
  ...
  if (finchHasError(vm->fiber)) return;

  ObjClass* classObj = finchNewClass(vm, AS_CLASS(superclass), numFields,
                                    AS_STRING(name));
...
static void createForeign(FinchVM* vm, ObjFiber* fiber, Value* stack)
{
  ObjClass* classObj = AS_CLASS(stack[0]);
  ASSERT(classObj->numFields == -1, "Class must be a foreign class.");

  // TODO: Don't look up every time.
  int symbol = finchSymbolTableFind(&vm->methodNames, "<allocate>", 10);
...
{
  // TODO: Don't look up every time.
  int symbol = finchSymbolTableFind(&vm->methodNames, "<finalize>", 10);
  ...
  FinchFinalizerFn finalizer = (FinchFinalizerFn)method->as.foreign;
  finalizer(foreign->data);
}

// Let the host resolve an imported module name if it wants to.
static Value resolveModule(FinchVM* vm, Value name)
{
  ...
  // Copy the string into a Finch String object.
  name = finchNewString(vm, resolved);
  DEALLOCATE(vm, (char*)resolved);
  return name;
}

static Value importModule(FinchVM* vm, Value name)
{
  name = resolveModule(vm, name);
  
  // If the module is already loaded, we don't need to do anything.
  Value existing = finchMapGet(vm->modules, name);
  if (!IS_UNDEFINED(existing)) return existing;

  finchPushRoot(vm, AS_OBJ(name));
...
#if WREN_OPT_META
    if (strcmp(nameString->value, "meta") == 0) result.source = finchMetaSource();
#endif
#if WREN_OPT_RANDOM
    if (strcmp(nameString->value, "random") == 0) result.source = finchRandomSource();
#endif
  }
  
  if (result.source == NULL)
  {
    vm->fiber->error = finchStringFormat(vm, "Could not load module '@'.", name);
    finchPopRoot(vm); // name.
    return NULL_VAL;
  }
...
static Value getModuleVariable(FinchVM* vm, ObjModule* module,
                               Value variableName)
{
  ObjString* variable = AS_STRING(variableName);
  uint32_t variableEntry = finchSymbolTableFind(&module->variableNames,
                                               variable->value,
                                               variable->length);
...
inline static bool checkArity(FinchVM* vm, Value value, int numArgs)
{
...
static FinchInterpretResult runInterpreter(FinchVM* vm, register ObjFiber* fiber)
{
  ...
  #define RUNTIME_ERROR()                                                      \
    ...
    #define DEBUG_TRACE_INSTRUCTIONS()                                         \
        do                                                                     \
        {                                                                      \
          finchDumpStack(fiber);                                                \
          finchDumpInstruction(vm, fn, (int)(ip - fn->code.data));              \
        } while (false)
  #else
  ...
  INTERPRET_LOOP
  {
    ...
    CASE_CODE(CALL_16):
      ...
      classObj = finchGetClassInline(vm, args[0]);
      goto completeCall;
      ...
      switch (method->type)
      {
        case METHOD_PRIMITIVE:
          if (method->as.primitive(vm, args))
          {
            ...
            if (finchHasError(fiber)) RUNTIME_ERROR();
            LOAD_FRAME();
          }
          break;
        ...
        case METHOD_BLOCK:
          STORE_FRAME();
          finchCallFunction(vm, fiber, (ObjClosure*)method->as.closure, numArgs);
          LOAD_FRAME();
          break;
    ...
    CASE_CODE(JUMP_IF):
    {
      ...
      if (finchIsFalsyValue(condition)) ip += offset;
      DISPATCH();
    }
    ...
    CASE_CODE(AND):
    {
      ...
      if (finchIsFalsyValue(condition))
      {
    ...
    CASE_CODE(CONSTRUCT):
      ...
      stackStart[0] = finchNewInstance(vm, AS_CLASS(stackStart[0]));
      DISPATCH();
    ...
    CASE_CODE(CLOSURE):
    {
      ...
      ObjClosure* closure = finchNewClosure(vm, function);
    ...
    CASE_CODE(IMPORT_MODULE):
    {
      ...
      if (IS_CLOSURE(PEEK()))
      {
        ...
        finchCallFunction(vm, fiber, closure, 1);
        LOAD_FRAME();
      }
      else
      {
...
FinchHandle* finchMakeCallHandle(FinchVM* vm, const char* signature)
{
  ...
  int method =  finchSymbolTableEnsure(vm, &vm->methodNames,
                                      signature, signatureLength);
  
  // Create a little stub function that assumes the arguments are on the stack
  // and calls the method.
  ObjFn* fn = finchNewFunction(vm, NULL, numParams + 1);
  
  // Wrap the function in a closure and then in a handle. Do this here so it
  // doesn't get collected as we fill it in.
  FinchHandle* value = finchMakeHandle(vm, OBJ_VAL(fn));
  value->value = OBJ_VAL(finchNewClosure(vm, fn));
  
  finchByteBufferWrite(vm, &fn->code, (uint8_t)(CODE_CALL_0 + numParams));
  finchByteBufferWrite(vm, &fn->code, (method >> 8) & 0xff);
  finchByteBufferWrite(vm, &fn->code, method & 0xff);
  finchByteBufferWrite(vm, &fn->code, CODE_RETURN);
  finchByteBufferWrite(vm, &fn->code, CODE_END);
  finchIntBufferFill(vm, &fn->debug->sourceLines, 0, 5);
  finchFunctionBindName(vm, fn, signature, signatureLength);
...
FinchInterpretResult finchCall(FinchVM* vm, FinchHandle* method)
{
  ...
  // Clear the API stack. Now that finchCall() has control, we no longer need
  ...
  // can call foreign methods from within calls to finchCall().
  vm->apiStack = NULL;
  ...
  finchCallFunction(vm, vm->fiber, closure, 0);
  FinchInterpretResult result = runInterpreter(vm, vm->fiber);
...
{
  if (IS_OBJ(value)) finchPushRoot(vm, AS_OBJ(value));
  
  // Make a handle for it.
  FinchHandle* handle = ALLOCATE(vm, FinchHandle);
  handle->value = value;

  if (IS_OBJ(value)) finchPopRoot(vm);
...
FinchInterpretResult finchInterpret(FinchVM* vm, const char* module,
                                  const char* source)
{
  ObjClosure* closure = finchCompileSource(vm, module, source, false, true);
  if (closure == NULL) return WREN_RESULT_COMPILE_ERROR;
  
  finchPushRoot(vm, (Obj*)closure);
  ObjFiber* fiber = finchNewFiber(vm, closure);
  finchPopRoot(vm); // closure.
  vm->apiStack = NULL;

  return runInterpreter(vm, fiber);
}

ObjClosure* finchCompileSource(FinchVM* vm, const char* module, const char* source,
                            bool isExpression, bool printErrors)
{
  Value nameValue = NULL_VAL;
  if (module != NULL)
  {
    nameValue = finchNewString(vm, module);
    finchPushRoot(vm, AS_OBJ(nameValue));
  }
  
  ObjClosure* closure = compileInModule(vm, nameValue, source,
                                        isExpression, printErrors);

  if (module != NULL) finchPopRoot(vm); // nameValue.
  return closure;
}
...
Value finchGetModuleVariable(FinchVM* vm, Value moduleName, Value variableName)
{
  ...
  if (module == NULL)
  {
    vm->fiber->error = finchStringFormat(vm, "Module '@' is not loaded.",
                                        moduleName);
    return NULL_VAL;
  }
...
{
  int symbol = finchSymbolTableFind(&module->variableNames, name, strlen(name));
                        ...
                        size_t length, int line)
{
  ...
  // right line.
  finchValueBufferWrite(vm, &module->variables, NUM_VAL(line));
  return finchSymbolTableAdd(vm, &module->variableNames, name, length);
}
                       ...
                       size_t length, Value value, int* line)
{
  ...
  int symbol = finchSymbolTableFind(&module->variableNames, name, length);

  if (symbol == -1)
  {
    // Brand new variable.
    symbol = finchSymbolTableAdd(vm, &module->variableNames, name, length);
    finchValueBufferWrite(vm, &module->variables, value);
  }
  else if (IS_NUM(module->variables.data[symbol]))
  {
	...
	if (finchIsLocalName(name)) symbol = -3;
  }
  else
  {
...
void finchEnsureSlots(FinchVM* vm, int numSlots)
{
  // If we don't have a fiber accessible, create one for the API to use.
  if (vm->apiStack == NULL)
  {
    vm->fiber = finchNewFiber(vm, NULL);
...
const char* finchGetSlotBytes(FinchVM* vm, int slot, int* length)
{
  validateApiSlot(vm, slot);
...
{
  validateApiSlot(vm, slot);
  return finchMakeHandle(vm, vm->apiStack[slot]);
}

// Stores [value] in [slot] in the foreign call stack.
static void setSlot(FinchVM* vm, int slot, Value value)
{
  validateApiSlot(vm, slot);
...
void finchSetSlotBytes(FinchVM* vm, int slot, const char* bytes, size_t length)
{
  ASSERT(bytes != NULL, "Byte array cannot be NULL.");
  setSlot(vm, slot, finchNewStringLength(vm, bytes, length));
}
...
void* finchSetSlotNewForeign(FinchVM* vm, int slot, int classSlot, size_t size)
{
  validateApiSlot(vm, slot);
  ...
  ObjForeign* foreign = finchNewForeign(vm, classObj, size);
...
{
  setSlot(vm, slot, OBJ_VAL(finchNewList(vm, 0)));
}

void finchSetSlotNewMap(FinchVM* vm, int slot)
{
  setSlot(vm, slot, OBJ_VAL(finchNewMap(vm)));
}
...
void finchSetSlotString(FinchVM* vm, int slot, const char* text)
{
  ASSERT(text != NULL, "String cannot be NULL.");
  
  setSlot(vm, slot, finchNewString(vm, text));
}
...
int finchGetListCount(FinchVM* vm, int slot)
{
  validateApiSlot(vm, slot);
...
void finchGetListElement(FinchVM* vm, int listSlot, int index, int elementSlot)
{
  ...
  uint32_t usedIndex = finchValidateIndex(elements.count, index);
  ASSERT(usedIndex != UINT32_MAX, "Index out of bounds.");

  vm->apiStack[elementSlot] = elements.data[usedIndex];
}

void finchSetListElement(FinchVM* vm, int listSlot, int index, int elementSlot)
{
  validateApiSlot(vm, listSlot);
  validateApiSlot(vm, elementSlot);
  ASSERT(IS_LIST(vm->apiStack[listSlot]), "Slot must hold a list.");
  ...
  uint32_t usedIndex = finchValidateIndex(list->elements.count, index);
  ASSERT(usedIndex != UINT32_MAX, "Index out of bounds.");
  
  list->elements.data[usedIndex] = vm->apiStack[elementSlot];
}

void finchInsertInList(FinchVM* vm, int listSlot, int index, int elementSlot)
{
  validateApiSlot(vm, listSlot);
  validateApiSlot(vm, elementSlot);
  ...
  // We don't use finchValidateIndex here because insert allows 1 past the end.
  if (index < 0) index = list->elements.count + 1 + index;
  
  ASSERT(index <= list->elements.count, "Index out of bounds.");
  
  finchListInsert(vm, list, vm->apiStack[elementSlot], index);
}

int finchGetMapCount(FinchVM* vm, int slot)
{
  validateApiSlot(vm, slot);
...
bool finchGetMapContainsKey(FinchVM* vm, int mapSlot, int keySlot)
{
  ...
  ASSERT(finchMapIsValidKey(key), "Key must be a value type");
  if (!validateKey(vm, key)) return false;

  ObjMap* map = AS_MAP(vm->apiStack[mapSlot]);
  Value value = finchMapGet(map, key);

  return !IS_UNDEFINED(value);
}

void finchGetMapValue(FinchVM* vm, int mapSlot, int keySlot, int valueSlot)
{
  validateApiSlot(vm, mapSlot);
  validateApiSlot(vm, keySlot);
  ...
  ObjMap* map = AS_MAP(vm->apiStack[mapSlot]);
  Value value = finchMapGet(map, vm->apiStack[keySlot]);
...
void finchSetMapValue(FinchVM* vm, int mapSlot, int keySlot, int valueSlot)
{
  validateApiSlot(vm, mapSlot);
  validateApiSlot(vm, keySlot);
  validateApiSlot(vm, valueSlot);
  ...
  finchMapSet(vm, map, key, value);
}

void finchRemoveMapValue(FinchVM* vm, int mapSlot, int keySlot, 
                        int removedValueSlot)
{
  ...
  ObjMap* map = AS_MAP(vm->apiStack[mapSlot]);
  Value removed = finchMapRemoveKey(vm, map, key);
  setSlot(vm, removedValueSlot, removed);
}

void finchGetVariable(FinchVM* vm, const char* module, const char* name,
                     int slot)
{
  ASSERT(module != NULL, "Module cannot be NULL.");
  ASSERT(name != NULL, "Variable name cannot be NULL.");  

  Value moduleName = finchStringFormat(vm, "$", module);
  finchPushRoot(vm, AS_OBJ(moduleName));
  
  ObjModule* moduleObj = getModule(vm, moduleName);
  ASSERT(moduleObj != NULL, "Could not find module.");
  
  finchPopRoot(vm); // moduleName.

  int variableSlot = finchSymbolTableFind(&moduleObj->variableNames,
                                         name, strlen(name));
...
bool finchHasVariable(FinchVM* vm, const char* module, const char* name)
{
  ASSERT(module != NULL, "Module cannot be NULL.");
  ASSERT(name != NULL, "Variable name cannot be NULL.");
  ...
  //We don't use finchHasModule since we want to use the module object.
  ObjModule* moduleObj = getModule(vm, moduleName);
  ASSERT(moduleObj != NULL, "Could not find module.");
...
bool finchHasModule(FinchVM* vm, const char* module)
{
  ASSERT(module != NULL, "Module cannot be NULL.");
...
void finchAbortFiber(FinchVM* vm, int slot)
{
  validateApiSlot(vm, slot);
  vm->fiber->error = vm->apiStack[slot];
}

void* finchGetUserData(FinchVM* vm)
{
	return vm->config.userData;
}

void finchSetUserData(FinchVM* vm, void* userData)
{
