                       ...
                       const char* format, va_list args)
{
  ...
  // Only report errors if there is a FinchErrorFn to handle them.
...
static int addConstant(Compiler* compiler, Value constant)
{
  ...
  {
    Value existing = finchMapGet(compiler->constants, constant);
  ...
  {
    if (IS_OBJ(constant)) finchPushRoot(compiler->parser->vm, AS_OBJ(constant));
    finchValueBufferWrite(compiler->parser->vm, &compiler->fn->constants,
                         constant);
    if (IS_OBJ(constant)) finchPopRoot(compiler->parser->vm);
    
    if (compiler->constants == NULL)
    {
      compiler->constants = finchNewMap(compiler->parser->vm);
    }
    finchMapSet(compiler->parser->vm, compiler->constants, constant,
                         ...
                         bool isMethod)
{
  ...
  compiler->attributes = finchNewMap(parser->vm);
  compiler->fn = finchNewFunction(parser->vm, parser->module,
                                 compiler->numLocals);
}
...
{
  ByteBuffer string;
  finchByteBufferInit(&string);
  finchByteBufferWrite(parser->vm, &string, firstChar);

  while (isName(peekChar(parser)) || isDigit(peekChar(parser)))
  {
    char c = nextChar(parser);
    finchByteBufferWrite(parser->vm, &string, c);
  }
  ...
  parser->next.value = finchNewStringLength(parser->vm,
                                            (char*)string.data, string.count);

  finchByteBufferClear(parser->vm, &string);
...
static void readUnicodeEscape(Parser* parser, ByteBuffer* string, int length)
{
  ...
  int numBytes = finchUtf8EncodeNumBytes(value);
  if (numBytes != 0)
  {
    finchByteBufferFill(parser->vm, string, 0, numBytes);
    finchUtf8Encode(value, string->data + string->count - numBytes);
  }
}
...
static void readString(Parser* parser)
{
  ...
  for (;;)
  {
    ...
    if (c == '\\')
    {
      ...
      {
        case '"':  finchByteBufferWrite(parser->vm, &string, '"'); break;
        case '\\': finchByteBufferWrite(parser->vm, &string, '\\'); break;
        case '%':  finchByteBufferWrite(parser->vm, &string, '%'); break;
        case '0':  finchByteBufferWrite(parser->vm, &string, '\0'); break;
        case 'a':  finchByteBufferWrite(parser->vm, &string, '\a'); break;
        case 'b':  finchByteBufferWrite(parser->vm, &string, '\b'); break;
        case 'e':  finchByteBufferWrite(parser->vm, &string, '\33'); break;
        case 'f':  finchByteBufferWrite(parser->vm, &string, '\f'); break;
        case 'n':  finchByteBufferWrite(parser->vm, &string, '\n'); break;
        case 'r':  finchByteBufferWrite(parser->vm, &string, '\r'); break;
        case 't':  finchByteBufferWrite(parser->vm, &string, '\t'); break;
        case 'u':  readUnicodeEscape(parser, &string, 4); break;
        case 'U':  readUnicodeEscape(parser, &string, 8); break;
        case 'v':  finchByteBufferWrite(parser->vm, &string, '\v'); break;
        case 'x':
          finchByteBufferWrite(parser->vm, &string,
      ...
      default:
        ...
        else
        {
          ...
          else
          {
            ...
            // meaningful code units in Finch, the lexer works on raw bytes,
...
{
  finchByteBufferWrite(compiler->parser->vm, &compiler->fn->code, (uint8_t)byte);
  
  // Assume the instruction is associated with the most recently consumed token.
  finchIntBufferWrite(compiler->parser->vm, &compiler->fn->debug->sourceLines,
...
static int declareVariable(Compiler* compiler, Token* token)
{
  ...
  {
    int line = -1;
    int symbol = finchDefineVariable(compiler->parser->vm,
...
static Variable resolveName(Compiler* compiler, const char* name, int length)
{
  ...
  variable.index = finchSymbolTableFind(&compiler->parser->module->variableNames,
                                       name, length);
  return variable;
}
                          ...
                          const char* debugName, int debugNameLength)
{
  ...
  finchFunctionBindName(compiler->parser->vm, compiler->fn,
  ...
  #if WREN_DEBUG_DUMP_COMPILED_CODE
    finchDumpCode(compiler->parser->vm, compiler->fn);
  #endif
...
{
  return finchSymbolTableEnsure(compiler->parser->vm,
...
{
  int symbol = finchSymbolTableFind(&compiler->parser->module->variableNames,
                                   name, strlen(name));
...
static void field(Compiler* compiler, bool canAssign)
{
  ...
  else
  {
    ...
    field = finchSymbolTableEnsure(compiler->parser->vm, &enclosingClass->fields,
...
static void name(Compiler* compiler, bool canAssign)
{
  ...
  // on this.
  if (finchIsLocalName(token->start) && getEnclosingClass(compiler) != NULL)
  {
    loadThis(compiler);
  ...
  if (variable.index == -1)
  {
    ...
    variable.index = finchDeclareVariable(compiler->parser->vm,
                                         compiler->parser->module,
                                         token->start, token->length,
                                         token->line);
...
static void forStatement(Compiler* compiler)
{
  ...
  // actually get names that aren't valid Finch identfiers, but that's the basic
  // idea.
  //
...
//
// Construction is a two-stage process in Finch that involves two separate
                         ...
                         const char* name, int length)
{
  ...
  finchIntBufferWrite(compiler->parser->vm, methods, symbol);
  return symbol;
}
...
static bool method(Compiler* compiler, Variable classVariable)
{
  ...
  if (isForeign)
  {
    ...
    emitConstant(compiler, finchNewStringLength(compiler->parser->vm,
...
static void classDefinition(Compiler* compiler, bool isForeign)
{
  ...
  Value classNameString = finchNewStringLength(compiler->parser->vm,
  ...
  classInfo.classAttributes = compiler->attributes->count > 0 
        ? finchNewMap(compiler->parser->vm) 
        : NULL;
  ...
  // bytecode will be adjusted by [finchBindMethod] to take inherited fields
  // into account.
  finchSymbolTableInit(&classInfo.fields);
  
  // Set up symbol buffers to track duplicate static and instance methods.
  finchIntBufferInit(&classInfo.methods);
  finchIntBufferInit(&classInfo.staticMethods);
  ...
  finchSymbolTableClear(compiler->parser->vm, &classInfo.fields);
  finchIntBufferClear(compiler->parser->vm, &classInfo.methods);
  finchIntBufferClear(compiler->parser->vm, &classInfo.staticMethods);
  compiler->enclosingClass = NULL;
  popScope(compiler);
}
...
static void import(Compiler* compiler)
{
  ...
  do
  {
    ...
    int sourceVariableConstant = addConstant(compiler,
          finchNewStringLength(compiler->parser->vm,
...
ObjFn* finchCompile(FinchVM* vm, ObjModule* module, const char* source,
...
void finchBindMethodCode(ObjClass* classObj, ObjFn* fn)
{
  int ip = 0;
  for (;;)
  {
    ...
    switch (instruction)
    {
      ...
      case CODE_CLOSURE:
      {
        ...
        finchBindMethodCode(classObj, AS_FN(fn->constants.data[constant]));
        break;
      }
...
void finchMarkCompiler(FinchVM* vm, Compiler* compiler)
{
  finchGrayValue(vm, compiler->parser->current.value);
  finchGrayValue(vm, compiler->parser->previous.value);
  finchGrayValue(vm, compiler->parser->next.value);

  // Walk up the parent chain to mark the outer compilers too. The VM only
  // tracks the innermost one.
  do
  {
    finchGrayObj(vm, (Obj*)compiler->fn);
    finchGrayObj(vm, (Obj*)compiler->constants);
    finchGrayObj(vm, (Obj*)compiler->attributes);
    
    if (compiler->enclosingClass != NULL)
    {
      finchBlackenSymbolTable(vm, &compiler->enclosingClass->fields);

      if(compiler->enclosingClass->methodAttributes != NULL) 
      {
        finchGrayObj(vm, (Obj*)compiler->enclosingClass->methodAttributes);
      }
      if(compiler->enclosingClass->classAttributes != NULL) 
      {
        finchGrayObj(vm, (Obj*)compiler->enclosingClass->classAttributes);
      }
    }
...
static void disallowAttributes(Compiler* compiler)
{
  if (compiler->numAttributes > 0)
  {
    ...
    finchMapClear(compiler->parser->vm, compiler->attributes);
    compiler->numAttributes = 0;
  }
}
...
{
  FinchVM* vm = compiler->parser->vm;

  if(IS_OBJ(group)) finchPushRoot(vm, AS_OBJ(group));
  if(IS_OBJ(key))   finchPushRoot(vm, AS_OBJ(key));
  if(IS_OBJ(value)) finchPushRoot(vm, AS_OBJ(value));

  Value groupMapValue = finchMapGet(compiler->attributes, group);
  if(IS_UNDEFINED(groupMapValue)) 
  {
    groupMapValue = OBJ_VAL(finchNewMap(vm));
    finchMapSet(vm, compiler->attributes, group, groupMapValue);
  }
  ...
  Value keyItemsValue = finchMapGet(groupMap, key);
  if(IS_UNDEFINED(keyItemsValue)) 
  {
    keyItemsValue = OBJ_VAL(finchNewList(vm, 0));
    finchMapSet(vm, groupMap, key, keyItemsValue);
  }

  //keyItems.add(value)
  ObjList* keyItems = AS_LIST(keyItemsValue);
  finchValueBufferWrite(vm, &keyItems->elements, value);

  if(IS_OBJ(group)) finchPopRoot(vm);
  if(IS_OBJ(key))   finchPopRoot(vm);
...
static void copyAttributes(Compiler* compiler, ObjMap* into)
{
  ...
  for(uint32_t attrIdx = 0; attrIdx < compiler->attributes->capacity; attrIdx++)
  {
    ...
    finchMapSet(vm, into, attrEntry->key, attrEntry->value);
  }
  
  finchMapClear(vm, compiler->attributes);
}
            ...
            bool isStatic, const char* fullSignature, int32_t length) 
{
  ...
  ObjMap* methodAttr = finchNewMap(vm);
  finchPushRoot(vm, (Obj*)methodAttr);
  ...
  if(compiler->enclosingClass->methodAttributes == NULL) {
    compiler->enclosingClass->methodAttributes = finchNewMap(vm);
  }
  
  // Store the method attributes in the class map
  Value key = finchNewStringLength(vm, fullSignatureWithPrefix, fullLength);
  finchMapSet(vm, compiler->enclosingClass->methodAttributes, key, OBJ_VAL(methodAttr));
