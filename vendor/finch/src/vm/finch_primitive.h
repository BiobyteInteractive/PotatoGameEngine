#ifndef finch_primitive_h
#define finch_primitive_h

#include "finch_vm.h"

// Binds a primitive method named [name] (in Finch) implemented using C function
// [fn] to `ObjClass` [cls].
#define PRIMITIVE(cls, name, function)                                         \
    do                                                                         \
    {                                                                          \
      int symbol = finchSymbolTableEnsure(vm,                                   \
          &vm->methodNames, name, strlen(name));                               \
      Method method;                                                           \
      method.type = METHOD_PRIMITIVE;                                          \
      method.as.primitive = prim_##function;                                   \
      finchBindMethod(vm, cls, symbol, method);                                 \
    } while (false)
...
#define DEF_PRIMITIVE(name)                                                    \
    static bool prim_##name(FinchVM* vm, Value* args)
...
#define RETURN_ERROR(msg)                                                      \
    do                                                                         \
    {                                                                          \
      vm->fiber->error = finchNewStringLength(vm, msg, sizeof(msg) - 1);        \
      return false;                                                            \
    } while (false)
...
#define RETURN_ERROR_FMT(...)                                                  \
    do                                                                         \
    {                                                                          \
      vm->fiber->error = finchStringFormat(vm, __VA_ARGS__);                    \
      return false;                                                            \
    } while (false)
