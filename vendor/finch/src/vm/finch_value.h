#ifndef finch_value_h
#define finch_value_h

#include <stdbool.h>
#include <string.h>

#include "finch_common.h"
#include "finch_math.h"
#include "finch_utils.h"

// This defines the built-in types and their core representations in memory.
// Since Finch is dynamically typed, any variable can hold a value of any type,
...
// storage location that can hold any Finch value. The stack, module variables,
...
// Finch implementation calls these "Obj", or objects, though to a user, all
// values are objects.
//
...
#define AS_NUM(value)       (finchValueToNum(value))             // double
...
// These macros promote a primitive C value to a full Finch Value. There are
...
#define NUM_VAL(num) (finchNumToValue(num))                      // double
#define OBJ_VAL(obj) (finchObjectToValue((Obj*)(obj)))           // Any Obj___*

// These perform type tests on a Value, returning `true` if the Value is of the
// given type.
#define IS_BOOL(value) (finchIsBool(value))                      // Bool
#define IS_CLASS(value) (finchIsObjType(value, OBJ_CLASS))       // ObjClass
#define IS_CLOSURE(value) (finchIsObjType(value, OBJ_CLOSURE))   // ObjClosure
#define IS_FIBER(value) (finchIsObjType(value, OBJ_FIBER))       // ObjFiber
#define IS_FN(value) (finchIsObjType(value, OBJ_FN))             // ObjFn
#define IS_FOREIGN(value) (finchIsObjType(value, OBJ_FOREIGN))   // ObjForeign
#define IS_INSTANCE(value) (finchIsObjType(value, OBJ_INSTANCE)) // ObjInstance
#define IS_LIST(value) (finchIsObjType(value, OBJ_LIST))         // ObjList
#define IS_MAP(value) (finchIsObjType(value, OBJ_MAP))           // ObjMap
#define IS_RANGE(value) (finchIsObjType(value, OBJ_RANGE))       // ObjRange
#define IS_STRING(value) (finchIsObjType(value, OBJ_STRING))     // ObjString
...
#define CONST_STRING(vm, text) finchNewStringLength((vm), (text), sizeof(text) - 1)
...
typedef struct sObjUpvalue
{
  ...
  // collected, but they are not first class Finch objects.
  Obj obj;
...
typedef bool (*Primitive)(FinchVM* vm, Value* args);
...
typedef enum
{
  ...
  // initial fiber used by a call to `finchCall()` or `finchInterpret()`.
  FIBER_ROOT,
...
// [stackStart].
static inline void finchAppendCallFrame(FinchVM* vm, ObjFiber* fiber,
...
static inline bool finchHasError(const ObjFiber* fiber)
{
...
static inline bool finchMapIsValidKey(Value arg);
...
// [needle].
uint32_t finchStringFind(ObjString* haystack, ObjString* needle,
                        uint32_t startIndex);

// Returns true if [a] and [b] represent the same string.
static inline bool finchStringEqualsCString(const ObjString* a,
...
//
// Unlike finchGetClassInline in finch_vm.h, this is not inlined. Inlining helps
...
static inline bool finchValuesSame(Value a, Value b)
{
#if WREN_NAN_TAGGING
...
// [IS_BOOL].
static inline bool finchIsBool(Value value)
{
#if WREN_NAN_TAGGING
...
static inline bool finchIsObjType(Value value, ObjType type)
{
  return IS_OBJ(value) && AS_OBJ(value)->type == type;
}

// Converts the raw object pointer [obj] to a [Value].
static inline Value finchObjectToValue(Obj* obj)
{
#if WREN_NAN_TAGGING
...
static inline double finchValueToNum(Value value)
{
#if WREN_NAN_TAGGING
  return finchDoubleFromBits(value);
#else
  return value.as.num;
#endif
}
...
static inline Value finchNumToValue(double num)
{
#if WREN_NAN_TAGGING
  return finchDoubleToBits(num);
#else
  Value value;
  value.type = VAL_NUM;
  value.as.num = num;
