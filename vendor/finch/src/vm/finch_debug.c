...
void finchDebugPrintStackTrace(FinchVM* vm)
{
  ...
  for (int i = fiber->numFrames - 1; i >= 0; i--)
  {
    ...
    // detail of what part of the core module is written in C and what is Finch.
...
static int dumpInstruction(FinchVM* vm, ObjFn* fn, int i, int* lastLine)
{
  int start = i;
  ...
  switch (code)
  {
    case CODE_CONSTANT:
    {
      ...
      finchDumpValue(fn->constants.data[constant]);
      printf("'\n");
      break;
    }
    ...
    case CODE_IMPORT_MODULE:
    {
      ...
      finchDumpValue(fn->constants.data[name]);
      printf("'\n");
      break;
    }
    ...
    case CODE_IMPORT_VARIABLE:
    {
      ...
      finchDumpValue(fn->constants.data[variable]);
      printf("'\n");
      break;
    }
