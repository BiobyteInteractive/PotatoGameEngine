...
// This header contains macros and defines used across the entire Finch
// implementation. In particular, it contains "configuration" defines that
// control how Finch works. Some of these are only used while hacking on Finch
// itself.
//
// This header is *not* intended to be included by code outside of Finch itself.

// Finch pervasively uses the C99 integer types (uint16_t, etc.) along with some
...
// If true, then Finch uses a NaN-tagged double for its core value
...
// These flags are useful for debugging and hacking on Finch itself. They are not
...
#define ALLOCATE(vm, type)                                                     \
    ((type*)finchReallocate(vm, NULL, 0, sizeof(type)))
...
// assertion fails, there is a bug in Finch.
//
