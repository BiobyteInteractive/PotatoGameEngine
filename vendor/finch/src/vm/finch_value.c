...
static void initObj(FinchVM* vm, Obj* obj, ObjType type, ObjClass* classObj)
{
  obj->type = type;
  obj->isDark = false;
...
ObjClass* finchNewSingleClass(FinchVM* vm, int numFields, ObjString* name)
{
  ...
  finchPushRoot(vm, (Obj*)classObj);
  finchMethodBufferInit(&classObj->methods);
  finchPopRoot(vm);

  return classObj;
}

void finchBindSuperclass(FinchVM* vm, ObjClass* subclass, ObjClass* superclass)
{
  ...
  {
    finchBindMethod(vm, subclass, i, superclass->methods.data[i]);
  }
}

ObjClass* finchNewClass(FinchVM* vm, ObjClass* superclass, int numFields,
                       ObjString* name)
{
  // Create the metaclass.
  Value metaclassName = finchStringFormat(vm, "@ metaclass", OBJ_VAL(name));
  finchPushRoot(vm, AS_OBJ(metaclassName));

  ObjClass* metaclass = finchNewSingleClass(vm, 0, AS_STRING(metaclassName));
  metaclass->obj.classObj = vm->classClass;

  finchPopRoot(vm);

  // Make sure the metaclass isn't collected when we allocate the class.
  finchPushRoot(vm, (Obj*)metaclass);

  // Metaclasses always inherit Class and do not parallel the non-metaclass
  // hierarchy.
  finchBindSuperclass(vm, metaclass, vm->classClass);

  ObjClass* classObj = finchNewSingleClass(vm, numFields, name);
  ...
  finchBindSuperclass(vm, classObj, superclass);
...
void finchBindMethod(FinchVM* vm, ObjClass* classObj, int symbol, Method method)
{
  ...
  if (symbol >= classObj->methods.count)
  {
    ...
    finchMethodBufferFill(vm, &classObj->methods, noMethod,
...
ObjClosure* finchNewClosure(FinchVM* vm, ObjFn* fn)
{
...
ObjFiber* finchNewFiber(FinchVM* vm, ObjClosure* closure)
{
  ...
  int stackCapacity = closure == NULL
      ? 1
      : finchPowerOf2Ceil(closure->fn->maxSlots + 1);
  ...
  if (closure != NULL)
  {
    ...
    finchAppendCallFrame(vm, fiber, closure, fiber->stack);
...
void finchEnsureStack(FinchVM* vm, ObjFiber* fiber, int needed)
{
  if (fiber->stackCapacity >= needed) return;
  
  int capacity = finchPowerOf2Ceil(needed);
  
  Value* oldStack = fiber->stack;
  fiber->stack = (Value*)finchReallocate(vm, fiber->stack,
...
ObjForeign* finchNewForeign(FinchVM* vm, ObjClass* classObj, size_t size)
{
...
ObjFn* finchNewFunction(FinchVM* vm, ObjModule* module, int maxSlots)
{
  FnDebug* debug = ALLOCATE(vm, FnDebug);
  debug->name = NULL;
  finchIntBufferInit(&debug->sourceLines);

  ObjFn* fn = ALLOCATE(vm, ObjFn);
  initObj(vm, &fn->obj, OBJ_FN, vm->fnClass);
  
  finchValueBufferInit(&fn->constants);
  finchByteBufferInit(&fn->code);
  fn->module = module;
...
void finchFunctionBindName(FinchVM* vm, ObjFn* fn, const char* name, int length)
{
...
Value finchNewInstance(FinchVM* vm, ObjClass* classObj)
{
...
ObjList* finchNewList(FinchVM* vm, uint32_t numElements)
{
...
void finchListInsert(FinchVM* vm, ObjList* list, Value value, uint32_t index)
{
  if (IS_OBJ(value)) finchPushRoot(vm, AS_OBJ(value));

  // Add a slot at the end of the list.
  finchValueBufferWrite(vm, &list->elements, NULL_VAL);
...
int finchListIndexOf(FinchVM* vm, ObjList* list, Value value)
{
  int count = list->elements.count;
  for (int i = 0; i < count; i++)
  {
    Value item = list->elements.data[i];
    if(finchValuesEqual(item, value)) {
      return i;
    }
  }
  return -1;
...
Value finchListRemoveAt(FinchVM* vm, ObjList* list, uint32_t index)
{
  Value removed = list->elements.data[index];

  if (IS_OBJ(removed)) finchPushRoot(vm, AS_OBJ(removed));
  ...
  {
    list->elements.data = (Value*)finchReallocate(vm, list->elements.data,
        sizeof(Value) * list->elements.capacity,
        sizeof(Value) * (list->elements.capacity / GROW_FACTOR));
    list->elements.capacity /= GROW_FACTOR;
  }

  if (IS_OBJ(removed)) finchPopRoot(vm);

  list->elements.count--;
  return removed;
}

ObjMap* finchNewMap(FinchVM* vm)
{
...
static inline uint32_t hashNumber(double num)
{
  ...
  return hashBits(finchDoubleToBits(num));
}
                      ...
                      MapEntry** result)
{
  ...
  do
  {
    ...
    else if (finchValuesEqual(entry->key, key))
    {
      // We found the key.
      *result = entry;
      return true;
...
static void resizeMap(FinchVM* vm, ObjMap* map, uint32_t capacity)
{
...
Value finchMapGet(ObjMap* map, Value key)
{
  MapEntry* entry;
...
void finchMapSet(FinchVM* vm, ObjMap* map, Value key, Value value)
{
...
void finchMapClear(FinchVM* vm, ObjMap* map)
{
  DEALLOCATE(vm, map->entries);
  map->entries = NULL;
  map->capacity = 0;
...
Value finchMapRemoveKey(FinchVM* vm, ObjMap* map, Value key)
{
  MapEntry* entry;
...
ObjModule* finchNewModule(FinchVM* vm, ObjString* name)
{
  ...
  finchSymbolTableInit(&module->variableNames);
  finchValueBufferInit(&module->variables);
...
Value finchNewRange(FinchVM* vm, double from, double to, bool isInclusive)
{
...
// hash.
static ObjString* allocateString(FinchVM* vm, size_t length)
{
...
Value finchNewString(FinchVM* vm, const char* text)
{
  return finchNewStringLength(vm, text, strlen(text));
}

Value finchNewStringLength(FinchVM* vm, const char* text, size_t length)
{
...
Value finchNewStringFromRange(FinchVM* vm, ObjString* source, int start,
                             uint32_t count, int step)
{
  ...
  {
    length += finchUtf8DecodeNumBytes(from[start + i * step]);
  }
  ...
  for (uint32_t i = 0; i < count; i++)
  {
    ...
    int codePoint = finchUtf8Decode(from + index, source->length - index);

    if (codePoint != -1)
    {
      to += finchUtf8Encode(codePoint, to);
    }
  }
...
Value finchNumToString(FinchVM* vm, double value)
{
  ...
  return finchNewStringLength(vm, buffer, length);
}

Value finchStringFromCodePoint(FinchVM* vm, int value)
{
  int length = finchUtf8EncodeNumBytes(value);
  ASSERT(length != 0, "Value out of range.");

  ObjString* string = allocateString(vm, length);

  finchUtf8Encode(value, (uint8_t*)string->value);
  hashString(string);

  return OBJ_VAL(string);
}

Value finchStringFromByte(FinchVM *vm, uint8_t value)
{
  int length = 1;
  ObjString* string = allocateString(vm, length);
...
Value finchStringFormat(FinchVM* vm, const char* format, ...)
{
  va_list argList;
...
Value finchStringCodePointAt(FinchVM* vm, ObjString* string, uint32_t index)
{
  ASSERT(index < string->length, "Index out of bounds.");

  int codePoint = finchUtf8Decode((uint8_t*)string->value + index,
                                 string->length - index);
  if (codePoint == -1)
  {
    ...
    bytes[1] = '\0';
    return finchNewStringLength(vm, bytes, 1);
  }

  return finchStringFromCodePoint(vm, codePoint);
}

// Uses the Boyer-Moore-Horspool string matching algorithm.
uint32_t finchStringFind(ObjString* haystack, ObjString* needle, uint32_t start)
{
...
ObjUpvalue* finchNewUpvalue(FinchVM* vm, Value* value)
{
...
void finchGrayObj(FinchVM* vm, Obj* obj)
{
  if (obj == NULL) return;
...
void finchGrayValue(FinchVM* vm, Value value)
{
  if (!IS_OBJ(value)) return;
  finchGrayObj(vm, AS_OBJ(value));
}

void finchGrayBuffer(FinchVM* vm, ValueBuffer* buffer)
{
  for (int i = 0; i < buffer->count; i++)
  {
    finchGrayValue(vm, buffer->data[i]);
  }
}

static void blackenClass(FinchVM* vm, ObjClass* classObj)
{
  // The metaclass.
  finchGrayObj(vm, (Obj*)classObj->obj.classObj);

  // The superclass.
  finchGrayObj(vm, (Obj*)classObj->superclass);
  ...
  for (int i = 0; i < classObj->methods.count; i++)
  {
    ...
    {
      finchGrayObj(vm, (Obj*)classObj->methods.data[i].as.closure);
    }
  }

  finchGrayObj(vm, (Obj*)classObj->name);

  if(!IS_NULL(classObj->attributes)) finchGrayObj(vm, AS_OBJ(classObj->attributes));
...
static void blackenClosure(FinchVM* vm, ObjClosure* closure)
{
  // Mark the function.
  finchGrayObj(vm, (Obj*)closure->fn);

  // Mark the upvalues.
  for (int i = 0; i < closure->fn->numUpvalues; i++)
  {
    finchGrayObj(vm, (Obj*)closure->upvalues[i]);
  }
...
static void blackenFiber(FinchVM* vm, ObjFiber* fiber)
{
  // Stack functions.
  for (int i = 0; i < fiber->numFrames; i++)
  {
    finchGrayObj(vm, (Obj*)fiber->frames[i].closure);
  }

  // Stack variables.
  for (Value* slot = fiber->stack; slot < fiber->stackTop; slot++)
  {
    finchGrayValue(vm, *slot);
  }
  ...
  while (upvalue != NULL)
  {
    finchGrayObj(vm, (Obj*)upvalue);
    upvalue = upvalue->next;
  }

  // The caller.
  finchGrayObj(vm, (Obj*)fiber->caller);
  finchGrayValue(vm, fiber->error);
...
static void blackenFn(FinchVM* vm, ObjFn* fn)
{
  // Mark the constants.
  finchGrayBuffer(vm, &fn->constants);
...
static void blackenForeign(FinchVM* vm, ObjForeign* foreign)
{
...
static void blackenInstance(FinchVM* vm, ObjInstance* instance)
{
  finchGrayObj(vm, (Obj*)instance->obj.classObj);

  // Mark the fields.
  for (int i = 0; i < instance->obj.classObj->numFields; i++)
  {
    finchGrayValue(vm, instance->fields[i]);
  }
...
static void blackenList(FinchVM* vm, ObjList* list)
{
  // Mark the elements.
  finchGrayBuffer(vm, &list->elements);
...
static void blackenMap(FinchVM* vm, ObjMap* map)
{
  // Mark the entries.
  for (uint32_t i = 0; i < map->capacity; i++)
  {
    ...
    finchGrayValue(vm, entry->key);
    finchGrayValue(vm, entry->value);
  }
...
static void blackenModule(FinchVM* vm, ObjModule* module)
{
  // Top-level variables.
  for (int i = 0; i < module->variables.count; i++)
  {
    finchGrayValue(vm, module->variables.data[i]);
  }

  finchBlackenSymbolTable(vm, &module->variableNames);

  finchGrayObj(vm, (Obj*)module->name);

  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjModule);
}

static void blackenRange(FinchVM* vm, ObjRange* range)
{
  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjRange);
}

static void blackenString(FinchVM* vm, ObjString* string)
{
  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjString) + string->length + 1;
}

static void blackenUpvalue(FinchVM* vm, ObjUpvalue* upvalue)
{
  // Mark the closed-over object (in case it is closed).
  finchGrayValue(vm, upvalue->closed);

  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjUpvalue);
}

static void blackenObject(FinchVM* vm, Obj* obj)
{
#if WREN_DEBUG_TRACE_MEMORY
  printf("mark ");
  finchDumpValue(OBJ_VAL(obj));
  printf(" @ %p\n", obj);
#endif
...
void finchBlackenObjects(FinchVM* vm)
{
...
void finchFreeObj(FinchVM* vm, Obj* obj)
{
#if WREN_DEBUG_TRACE_MEMORY
  printf("free ");
  finchDumpValue(OBJ_VAL(obj));
  ...
  {
    case OBJ_CLASS:
      finchMethodBufferClear(vm, &((ObjClass*)obj)->methods);
      break;
    ...
    {
      ObjFn* fn = (ObjFn*)obj;
      finchValueBufferClear(vm, &fn->constants);
      finchByteBufferClear(vm, &fn->code);
      finchIntBufferClear(vm, &fn->debug->sourceLines);
    ...
    case OBJ_FOREIGN:
      finchFinalizeForeign(vm, (ObjForeign*)obj);
      break;

    case OBJ_LIST:
      finchValueBufferClear(vm, &((ObjList*)obj)->elements);
      break;
    ...
    case OBJ_MODULE:
      finchSymbolTableClear(vm, &((ObjModule*)obj)->variableNames);
      finchValueBufferClear(vm, &((ObjModule*)obj)->variables);
      break;
...
ObjClass* finchGetClass(FinchVM* vm, Value value)
{
  return finchGetClassInline(vm, value);
}

bool finchValuesEqual(Value a, Value b)
{
  if (finchValuesSame(a, b)) return true;
  ...
  switch (aObj->type)
  {
    ...
    case OBJ_STRING:
    {
      ...
      finchStringEqualsCString(aString, bString->value, bString->length);
    }
