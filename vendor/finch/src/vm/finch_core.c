...
DEF_PRIMITIVE(fiber_new)
{
  ...
  RETURN_OBJ(finchNewFiber(vm, closure));
}
...
static bool runFiber(FinchVM* vm, ObjFiber* fiber, Value* args, bool isCall,
                     bool hasValue, const char* verb)
{

  if (finchHasError(fiber))
  {
...
DEF_PRIMITIVE(fiber_isDone)
{
  ...
  RETURN_BOOL(runFiber->numFrames == 0 || finchHasError(runFiber));
}
...
DEF_PRIMITIVE(fiber_try)
{
  ...
  if (!finchHasError(vm->fiber)) vm->fiber->state = FIBER_TRY;
  return false;
}
...
static void call_fn(FinchVM* vm, Value* args, int numArgs)
{
  // +1 to include the function itself.
  finchCallFunction(vm, vm->fiber, AS_CLOSURE(args[0]), numArgs + 1);
}
...
DEF_PRIMITIVE(list_filled)
{
  ...
  ObjList* list = finchNewList(vm, size);
...
{
  RETURN_OBJ(finchNewList(vm, 0));
}

DEF_PRIMITIVE(list_add)
{
  finchValueBufferWrite(vm, &AS_LIST(args[0])->elements, args[1]);
  RETURN_VAL(args[1]);
}
...
{
  finchValueBufferClear(vm, &AS_LIST(args[0])->elements);
  RETURN_NULL;
}
...
DEF_PRIMITIVE(list_insert)
{
  ...
  finchListInsert(vm, list, args[2], index);
  RETURN_VAL(args[2]);
}
...
DEF_PRIMITIVE(list_removeAt)
{
  ...
  RETURN_VAL(finchListRemoveAt(vm, list, index));
}

DEF_PRIMITIVE(list_removeValue) {
  ObjList* list = AS_LIST(args[0]);
  int index = finchListIndexOf(vm, list, args[1]);
...
{
  ObjList* list = AS_LIST(args[0]);
  RETURN_NUM(finchListIndexOf(vm, list, args[1]));
}
...
DEF_PRIMITIVE(list_subscript)
{
  ...
  ObjList* result = finchNewList(vm, count);
...
DEF_PRIMITIVE(map_subscript)
{
  ...
  Value value = finchMapGet(map, args[1]);
...
DEF_PRIMITIVE(map_subscriptSetter)
{
  ...
  finchMapSet(vm, AS_MAP(args[0]), args[1], args[2]);
  RETURN_VAL(args[2]);
}
...
{
  finchMapClear(vm, AS_MAP(args[0]));
  RETURN_NULL;
}
...
DEF_PRIMITIVE(map_containsKey)
{
  ...
  RETURN_BOOL(!IS_UNDEFINED(finchMapGet(AS_MAP(args[0]), args[1])));
}
...
DEF_PRIMITIVE(map_remove)
{
  ...
  RETURN_VAL(finchMapRemoveKey(vm, AS_MAP(args[0]), args[1]));
}
...
DEF_PRIMITIVE(num_dotDot)
{
  ...
  RETURN_VAL(finchNewRange(vm, from, to, true));
}
...
DEF_PRIMITIVE(num_dotDotDot)
{
  ...
  double from = AS_NUM(args[0]);
  double to = AS_NUM(args[1]);
  RETURN_VAL(finchNewRange(vm, from, to, false));
}
...
{
  RETURN_VAL(finchNumToString(vm, AS_NUM(args[0])));
}
...
{
  RETURN_BOOL(finchValuesEqual(args[1], args[2]));
}
...
{
  RETURN_BOOL(finchValuesEqual(args[0], args[1]));
}
...
DEF_PRIMITIVE(object_is)
{
  ...
  ObjClass *classObj = finchGetClass(vm, args[0]);
...
DEF_PRIMITIVE(object_toString)
{
  ...
  RETURN_VAL(finchStringFormat(vm, "instance of @", name));
}

DEF_PRIMITIVE(object_type)
{
  RETURN_OBJ(finchGetClass(vm, args[0]));
}
...
DEF_PRIMITIVE(range_toString)
{
  ...
  Value from = finchNumToString(vm, range->from);
  finchPushRoot(vm, AS_OBJ(from));

  Value to = finchNumToString(vm, range->to);
  finchPushRoot(vm, AS_OBJ(to));

  Value result = finchStringFormat(vm, "@$@", from,
...
DEF_PRIMITIVE(string_fromCodePoint)
{
  ...
  RETURN_VAL(finchStringFromCodePoint(vm, codePoint));
}
...
DEF_PRIMITIVE(string_fromByte)
{
  ...
  RETURN_VAL(finchStringFromByte(vm, (uint8_t) byte));
}
...
DEF_PRIMITIVE(string_codePointAt)
{
  ...
  RETURN_NUM(finchUtf8Decode((uint8_t*)string->value + index,
                            string->length - index));
}
...
DEF_PRIMITIVE(string_contains)
{
  ...
  RETURN_BOOL(finchStringFind(string, search, 0) != UINT32_MAX);
}
...
DEF_PRIMITIVE(string_indexOf1)
{
  ...
  uint32_t index = finchStringFind(string, search, 0);
...
DEF_PRIMITIVE(string_indexOf2)
{
  ...
  uint32_t index = finchStringFind(string, search, start);
  RETURN_NUM(index == UINT32_MAX ? -1 : (int)index);
}
...
DEF_PRIMITIVE(string_iteratorValue)
{
  ...
  RETURN_VAL(finchStringCodePointAt(vm, string, index));
}
...
DEF_PRIMITIVE(string_plus)
{
  ...
  RETURN_VAL(finchStringFormat(vm, "@@", args[0], args[1]));
}
...
DEF_PRIMITIVE(string_subscript)
{
  ...
  RETURN_VAL(finchNewStringFromRange(vm, string, start, count, step));
}
...
static ObjClass* defineClass(FinchVM* vm, ObjModule* module, const char* name)
{
  ObjString* nameString = AS_STRING(finchNewString(vm, name));
  finchPushRoot(vm, (Obj*)nameString);

  ObjClass* classObj = finchNewSingleClass(vm, 0, nameString);

  finchDefineVariable(vm, module, name, nameString->length, OBJ_VAL(classObj), NULL);

  finchPopRoot(vm);
  return classObj;
}

void finchInitializeCore(FinchVM* vm)
{
  ObjModule* coreModule = finchNewModule(vm, NULL);
  finchPushRoot(vm, (Obj*)coreModule);
  
  // The core module's key is null in the module map.
  finchMapSet(vm, vm->modules, NULL_VAL, OBJ_VAL(coreModule));
  finchPopRoot(vm); // coreModule.
  ...
  finchBindSuperclass(vm, vm->classClass, vm->objectClass);
  ...
  // collected.
  finchBindSuperclass(vm, objectMetaclass, vm->classClass);
  ...
  finchInterpret(vm, NULL, coreModuleSource);

  vm->boolClass = AS_CLASS(finchFindVariable(vm, coreModule, "Bool"));
  PRIMITIVE(vm->boolClass, "toString", bool_toString);
  PRIMITIVE(vm->boolClass, "!", bool_not);

  vm->fiberClass = AS_CLASS(finchFindVariable(vm, coreModule, "Fiber"));
  ...
  vm->fnClass = AS_CLASS(finchFindVariable(vm, coreModule, "Fn"));
  ...
  vm->nullClass = AS_CLASS(finchFindVariable(vm, coreModule, "Null"));
  PRIMITIVE(vm->nullClass, "!", null_not);
  PRIMITIVE(vm->nullClass, "toString", null_toString);

  vm->numClass = AS_CLASS(finchFindVariable(vm, coreModule, "Num"));
  ...
  vm->stringClass = AS_CLASS(finchFindVariable(vm, coreModule, "String"));
  ...
  vm->listClass = AS_CLASS(finchFindVariable(vm, coreModule, "List"));
  ...
  vm->mapClass = AS_CLASS(finchFindVariable(vm, coreModule, "Map"));
  ...
  vm->rangeClass = AS_CLASS(finchFindVariable(vm, coreModule, "Range"));
  ...
  ObjClass* systemClass = AS_CLASS(finchFindVariable(vm, coreModule, "System"));
