...
#include "finch_opt_random.finch.inc"
...
static void randomAllocate(FinchVM* vm)
{
  Well512* well = (Well512*)finchSetSlotNewForeign(vm, 0, 0, sizeof(Well512));
  well->index = 0;
}

static void randomSeed0(FinchVM* vm)
{
  Well512* well = (Well512*)finchGetSlotForeign(vm, 0);
...
static void randomSeed1(FinchVM* vm)
{
  Well512* well = (Well512*)finchGetSlotForeign(vm, 0);

  srand((uint32_t)finchGetSlotDouble(vm, 1));
  for (int i = 0; i < 16; i++)
  {
    well->state[i] = rand();
  }
...
static void randomSeed16(FinchVM* vm)
{
  Well512* well = (Well512*)finchGetSlotForeign(vm, 0);

  for (int i = 0; i < 16; i++)
  {
    well->state[i] = (uint32_t)finchGetSlotDouble(vm, i + 1);
  }
}

static void randomFloat(FinchVM* vm)
{
  Well512* well = (Well512*)finchGetSlotForeign(vm, 0);
...
static void randomInt0(FinchVM* vm)
{
  Well512* well = (Well512*)finchGetSlotForeign(vm, 0);

  finchSetSlotDouble(vm, 0, (double)advanceState(well));
}
