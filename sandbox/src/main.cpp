...
void OnDeviceFound(WGPURequestDeviceStatus status, WGPUDevice dev, char const *message, void *pUserData) {
...
void OnAdapterFound(WGPURequestAdapterStatus status, WGPUAdapter adapter, char const *message, void *pUserData) {
...
void CreateRenderPipeline() {
    ...
    fn fs_main() -> @location(0) vec4f {
        return vec4f(0.93, 0.42, 0.3, 1.0);
    }
    )";
...
void Render() {
    std::vector<wgpu::RenderPassColorAttachment> attachment = {
        {
            ...
            .clearValue = {0.35f, 0.39f, 0.61f, 1.0f},
        },
    };
    ...
    pass.SetVertexBuffer(0, buffer, 0, 3 * sizeof(glm::vec2));
    pass.Draw(3);
    pass.End();
...
int main() {
    ...
    window = glfwCreateWindow(kWidth, kHeight, "WebGPU window - Dawn", nullptr, nullptr);
    if (!window) {
